...
import { getAllTipoSector } from "../../api/tipoSector.api";
import {
  PencilSquareIcon,
  ChevronLeftIcon,
  ChevronRightIcon,
...
export default function GestionarProvincia() {
  ...
  useEffect(() => {
    async function fetchTable() {
      const { data } = await getAllTipoSector();
      setMyTable(data);
    }
    fetchTable();
  }, []);
        ...
        <div className="bg-white px-4 pt-3 pb-4 rounded-sm border border-gray-200 flex-1 ml-4 mr-4">
          <div className="flex justify-between items-center">
            ...
            <input
              ...
              placeholder="Buscar tipo sector"
              onKeyUp={(e) => {
                setSearchTerm(e.target.value);
              }}
            />
            <button
              onClick={() => navigate("/GestionarTipoSectorForm")}
                  ...
                  <tr key={`data${index}`} className="border text-left pb-2">
                    ...
                    <td className="">
                      <button
                        onClick={() =>
                          navigate(
                            `/gestionarTipoSectorForm/${dataRow.tipoSector}`
                          )
                        }
                      >
